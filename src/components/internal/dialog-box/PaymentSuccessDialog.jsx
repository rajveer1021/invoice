...
import { CheckCircle as CheckCircleIcon, Receipt as ReceiptIcon, EventAvailable as EventIcon } from '@mui/icons-material';
import { useNavigate } from 'react-router-dom';

const PaymentSuccessDialog = ({ open, onClose, selectedPlan, paymentId }) => {
    ...
    >
      <DialogContent sx={{ p: { xs: 3, sm: 6 }, textAlign: 'center' }}>
        <Box sx={{ 
          ...
          {isTrial ? (
            <EventIcon sx={{ fontSize: { xs: 50, sm: 60 }, color: '#1976D2' }} />
          ) : (
        ...
        <Typography variant="subtitle1" sx={{ mb: 3, color: 'text.secondary', maxWidth: 400, mx: 'auto', lineHeight: 1.6 }}>
          {isTrial ? (
            <>You're now subscribed to the <strong>{paymentDetails.planName}</strong>. Start exploring our features!</>
          ) : (
